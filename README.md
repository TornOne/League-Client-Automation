...
* `thirdPartyInterface`: An integer value specifying which third-party site to use for data for suggestions. A value of `1` uses LolAlytics. A value of `2` also uses LolAlytics, but their new API instead. You may have to switch to that if `1` ever stops working. Any other value effectively disables all functionality of the program except for preset rune pages.
