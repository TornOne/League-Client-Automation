...
class Program {
	...
	static async Task Main() {
		...
		while (true) {
			...
			} else if (command[0] == "http") {
				if (command[1] == "get") {
					string result = http.GetStringAsync(command[2]).Result;
					if (result.Length > 0) {
						Console.WriteLine(Json.PrettyPrint(result));
					}
				} else if (command[1] == "delete") {
					string result = await http.DeleteAsync(command[2]).Result.Content.ReadAsStringAsync();
					if (result.Length > 0) {
						Console.WriteLine(Json.PrettyPrint(result));
					}
				} else if (command[1] == "put") {
					StringContent request = new StringContent(command[3]);
					request.Headers.ContentType.MediaType = "application/json";
					string result = await http.PutAsync(command[2], request).Result.Content.ReadAsStringAsync();
					if (result.Length > 0) {
						Console.WriteLine(Json.PrettyPrint(result));
					}
					...
					StringContent request = new StringContent(command[3]);
					request.Headers.ContentType.MediaType = "application/json";
					string result = await http.PostAsync(command[2], request).Result.Content.ReadAsStringAsync();
					if (result.Length > 0) {
						Console.WriteLine(Json.PrettyPrint(result));
					}
				} else {
					Console.WriteLine($"Unknown HTTP command \"{command[1]}\"");
				}
